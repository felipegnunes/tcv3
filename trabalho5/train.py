...
def discriminator(X, reuse = None):
	with tf.variable_scope('discriminator', reuse = reuse):
		...
		fc1 = tf.layers.dense(inputs = fc1, units = 256, use_bias = False)
		fc1 = tf.nn.leaky_relu(features = fc1, alpha = 0.01)
		
		fc2 = tf.layers.dense(inputs = fc1, units = 128, use_bias = False)
		...
		#print(X.shape)
		#conv1 = tf.layers.conv2d(inputs = X, filters = 8, kernel_size = (3, 3), strides = (2, 2), padding = 'same', use_bias = False) 
		#conv1 = tf.layers.batch_normalization(conv1, training = is_training)
		#conv1 = tf.nn.leaky_relu(features = conv1, alpha = 0.01)
		#print(conv1.shape)
		
		#conv2 = tf.layers.conv2d(inputs = conv1, filters = 16, kernel_size = (2, 2), strides = (2, 2), padding = 'same', use_bias = False)
		#conv2 = tf.layers.batch_normalization(conv2, training = is_training)
		#conv2 = tf.nn.leaky_relu(features = conv2, alpha = 0.01)
		#print(conv2.shape)
		
		#conv3 = tf.layers.conv2d(inputs = conv2, filters = 32, kernel_size = (2, 2), strides = (2, 2), padding = 'same', use_bias = False)
		#conv3 = tf.layers.batch_normalization(conv3, training = is_training)
		#conv3 = tf.nn.leaky_relu(features = conv3, alpha = 0.01)
		#print(conv3.shape)
		
		#fc1 = tf.reshape(tensor = conv3, shape = (-1, conv3.shape[1] * conv3.shape[2] * conv3.shape[3]))
...
def generator(z, reuse = None):
	with tf.variable_scope('generator', reuse = reuse):	
		fc1 = tf.layers.dense(inputs = z, units = 128, use_bias = False)
		fc1 = tf.layers.batch_normalization(fc1, training = is_training)
		fc1 = tf.nn.leaky_relu(features = fc1, alpha = 0.01)
		print(fc1.shape)
		
		fc2 = tf.layers.dense(inputs = fc1, units = 256, use_bias = False)
		fc2 = tf.layers.batch_normalization(fc2, training = is_training)
		fc2 = tf.nn.leaky_relu(features = fc2, alpha = 0.01)
		print(fc2.shape)
		
		fc3 = tf.layers.dense(inputs = fc2, units = 512, use_bias = False)
		fc3 = tf.layers.batch_normalization(fc3, training = is_training)
		fc3 = tf.nn.leaky_relu(features = fc3, alpha = 0.01)
		print(fc3.shape)
		
		fc4 = tf.layers.dense(inputs = fc3, units = 1024, use_bias = False)
		fc4 = tf.reshape(tensor = fc4, shape = (-1, 32, 32, 1))
		print(fc4.shape)
		print()
		
		return tf.sigmoid(fc4)
...
with tf.Session() as sess:
	...
	for epoch in range(1, 5000):
		...
		cv2.imwrite('/home/nunes/tcv3/trabalho5/results/' + str(epoch) + '.png', gen_sample)	
